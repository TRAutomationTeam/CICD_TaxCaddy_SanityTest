...
# Determine authentication method and validate parameters
$authMethod = ""

if ($accountForApp -ne "" -and $applicationId -ne "" -and $applicationSecret -ne "" -and $applicationScope -ne "") {
    $authMethod = "ExternalAppOAuth"
}
elseif ($accountName -ne "" -and $userKey -ne "") {
    $authMethod = "ApiAccess"
}
elseif ($orchestrator_user -ne "" -and $orchestrator_pass -ne "") {
    $authMethod = "UsernamePassword"
}
else {
    WriteLog "Fill the required parameters for one authentication method (External App OAuth, API Access, or Username & Password)"
    exit 1
}
...
# Add authentication parameters based on the selected method
switch ($authMethod) {
    "ExternalAppOAuth" {
        WriteLog "Using External App OAuth authentication"
    ...
    "ApiAccess" {
        WriteLog "Using API Access authentication"
        $ParamList.Add("--token")
