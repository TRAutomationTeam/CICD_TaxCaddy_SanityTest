...
# Authentication Logic:  Exactly ONE of these methods MUST be used.
if ($accountForApp -ne "" -and $applicationId -ne "" -and $applicationSecret -ne "" -and $applicationScope -ne "") {
    # External App OAuth is provided
}
...
WriteLog "Executing $uipathCLI $($ParamMask -join ' ')" # Corrected logging
WriteLog "-----------------------------------------------------------------------------"
